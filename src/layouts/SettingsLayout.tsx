  ...
  return (
    <div className="w-full h-full flex flex-col">
      ...
      <div className="w-full bg-scry-bg dark:bg-scry-bg-dark border-y border-border dark:border-border-dark p-2 flex items-center justify-center gap-2">
        <Button onClick={() => navigate('/settings/general')} variant={currentPage == 'general' ? 'selectedTab' : 'tab'} className="justify-start">
          <Settings size={16} strokeWidth={1.5} /> General
        </Button>
        <Button onClick={() => navigate('/settings/profile')} variant={currentPage == 'profile' ? 'selectedTab' : 'tab'} className="justify-start">
        ...
        <Button onClick={() => navigate('/settings/about')} variant={currentPage == 'about' ? 'selectedTab' : 'tab'} className="justify-start">
          <FileCog size={16} strokeWidth={1.5} /> About
        </Button>
      </div>
      <div className="flex-1 min-h-0">{children}</div>
      {currentPage != 'about' && <div className="p-2 h-12 mt-auto border-t border-border dark:border-border-dark"></div>}
    </div>
  )
}
