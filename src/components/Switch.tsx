...
const Switch: React.FC<ToggleProps> = ({ className, name, onText, offText, reversed, checked, onChange }) => {
  ...
  return (
    <>
      <label className={twMerge('relative inline-flex gap-2 cursor-pointer select-none items-center', className, reversed ? 'flex-row-reverse' : '')}>
        <input type="checkbox" name={name} className="sr-only" checked={checked} onChange={handleCheckboxChange} />
        <span
          className={twMerge(
        ...
        <span className="flex items-center flex-1">
          <span className={`transition-all duration-400 ${checked ? '' : 'text-text-muted'}`}>{checked ? onText : offText}</span>
        </span>
      </label>
    </>
  )
